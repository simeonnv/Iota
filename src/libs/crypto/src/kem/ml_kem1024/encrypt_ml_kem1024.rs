...
pub fn encrypt_ml_kem1024(
    ...
    let public_key = match kem_alg.public_key_from_bytes(public_key) {
        Some(e) => e,
        None => return Err(Error::Internal("invalid encryption key".into())),
    };

    let (kem_ciphertext, shared_secret) = kem_alg.encapsulate(public_key)?;
